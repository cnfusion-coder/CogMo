...
const outcomePrefix = computed(() => {
  const c = props.outcome;
  let res = '';
  switch (c.prefixmark) {
    case '{':
    case 'LEFT': {
...
const idText = computed(() => `KNO ${props.outcome.oknowledge.id}`);

const outcomeText = computed(() => {
  if (props.expend) {
    return props.outcome.oknowledge.title
  } else {
    return idText.value
  }
});

const tagColor = computed(() => parseTrunkType(props.outcome.oknowledge).tint)

</script>

<template>
  <div style="display: flex; align-items: center; gap: 8px">
    <div v-if="outcomePrefix !== ''">
      {{ outcomePrefix }}
    </div>
    <el-popover width="180px">
      <template #reference>
        <ScrollingBadge :color="tagColor" :label="outcomeText"/>
      </template>
      <div style="display: flex; flex-direction: column; gap: 2px; align-items: start">
        ...
        <div>
          {{ props.outcome.oknowledge.title }}
        </div>
        ...
        <div>
          {{ props.outcome.oknowledge.description }}
        </div>
        <div style="height: 6px"/>
        <div :style="{color: tagColor}" style="font-weight: bold; align-self: center">
          {{ props.outcome.oknowledge.trunktype.name }} {{ i18nStore.t('600.module') || '模块' }}
        </div>
        <div style="height: 6px"/>
