...
function register() {
  ...
  baseServer.post('/api/user/register/', inputForm)
      .then(() => {
        baseServer.post('/api/user/login/', {
          ...
          password: inputForm.password
            ...
            .then(response => {
              if (response.status === 200) {
                ...
                router.push('/home');
              } else throw new Error('token 获取失败！')
            })
            .catch(e => {
              console.log(e)
      ...
      .catch(e => {
        if (e.response && e.response.data) {
          ...
          for (const [key, value] of Object.entries(e.response.data)) {
            switch (key) {
              case 'username': {
                reason += `用户名不合规：${value.join('; ')}`;
                break;
              }
              case 'password': {
                reason += `密码不合规：${value.join('; ')}`;
                break;
              }
              default: {
                reason += `${key} 不合规：${value.join('; ')}`;
                break;
              }
            }
          }
...
const userChoiceClass = computed(() => ({
  'active-choice': inputForm.role === 'staff',
  'disable-choice': inputForm.role !== 'staff',
  'img-father': true
}));

const normalAdminChoiceClass = computed(() => ({
  'active-choice': inputForm.role === 'admin',
  'disable-choice': inputForm.role !== 'admin',
  'img-father': true
}));

const superAdminChoiceClass = computed(() => ({
  'active-choice': inputForm.role === 'super',
  'disable-choice': inputForm.role !== 'super',
  'img-father': true
}));

function switchToRole(index) {
  if (waitingForResponse.value) return;
  inputForm.role = index;
}
...
<template>
  <el-row class="horizontal-center-container">
    <el-col :span="16" class="vertical-center horizontal-container">
      <div :class="userChoiceClass" @click="switchToRole('staff')">
      ...
      <div :class="normalAdminChoiceClass" @click="switchToRole('admin')">
      ...
      <div :class="superAdminChoiceClass" @click="switchToRole('super')">
      ...
      <div style="width: 100%; display: flex; flex-direction: column; align-items: center; gap: 8px">
        ...
        <el-form :model="inputForm">
          ...
          <el-form-item label="用户名" prop="username">
            <el-input v-model="inputForm.username" style="width: 100%;"></el-input>
          </el-form-item>

          <el-form-item label="密码" prop="password">
            <el-input type="password" show-password v-model="inputForm.password" style="width: 100%;"></el-input>
          </el-form-item>

          <el-form-item label="邮箱" prop="email">
            <el-input v-model="inputForm.email" style="width: 100%;"></el-input>
          </el-form-item>

          <div style="display: flex; flex-direction: column; gap: 8px">
            <el-button :disabled="waitingForResponse" :loading="waitingForResponse" style="width: 100%" class="full-width" type="primary" :color="visual.color.primary" dark @click="register">
              注册
            </el-button>
            <el-button class="full-width" @click="login" type="primary" :color="visual.color.primary"
                       plain dark style="margin-left: 0; width: 100%">
              返回登录
            </el-button>
          </div>
        </el-form>
